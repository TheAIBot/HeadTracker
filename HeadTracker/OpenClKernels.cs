...
kernel void GetClusterMap(global char* colors, int colorCount, global char* labPixels, global ushort* clusterMap)
{
    int index = get_global_id(0);

    float L = convert_float(labPixels[index * 3 + 0]);
    float a = convert_float(labPixels[index * 3 + 1]);
    float b = convert_float(labPixels[index * 3 + 2]);

    int bestColorIndex = 0;
    float bestDistance = 10000;
    for (int i = 0; i < colorCount; i++)
    {
        float cL = convert_float(colors[i * 3 + 0]);
        float ca = convert_float(colors[i * 3 + 1]);
        float cb = convert_float(colors[i * 3 + 2]);

        float distance = DistanceCIE94(L, a, b, cL, ca, cb);

        int isDistanceNaN = isnan(distance);
        
        float notNanDistance = (isnan(distance) == 1) ? 100000 : distance;
        float notInfDistance = (isinf(distance) == 1) ? 100000 : notNanDistance;
    ...
    clusterMap[index] = convert_ushort(bestColorIndex);
}";
    }
}
